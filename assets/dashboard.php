<?php
    ...
    // Total Contratado Passagem Aérea Nacional
    ...
    // Total Contratado Passagem Aérea Internacional
    ...
    // Total Contratado Passagem Rodoviária Nacional
    ...
    // Total Contratado Seguro Viagem Internacional
    ...
    // Total Aditado Passagem Aérea Nacional
        $result_adpan="SELECT SUM(valor_aditivo) as aditado_pan FROM tbl_aditivo 
        INNER JOIN tbl_item
        ON tbl_aditivo.id_item = tbl_item.id_item WHERE descricao LIKE 'Passagem Aérea Nacional' AND tbl_aditivo.status_aditivo LIKE 'ativo'";
        $resultado_adpan = $conn->prepare($result_adpan);
        $resultado_adpan->execute();
        $row_adpan = $resultado_adpan->fetch(PDO::FETCH_ASSOC);
        $aditadopan = $row_adpan['aditado_pan'];
        $adtpan =  number_format($aditadopan, 2, ',', '.'); // <--- Total Aditado PAN

    // Total Aditado Passagem Aérea Internacional
        $result_adpai="SELECT SUM(valor_aditivo) as aditado_pai FROM tbl_aditivo 
        INNER JOIN tbl_item
        ON tbl_aditivo.id_item = tbl_item.id_item WHERE descricao LIKE 'Passagem Aérea Internacional' AND tbl_aditivo.status_aditivo LIKE 'ativo'";
        $resultado_adpai = $conn->prepare($result_adpai);
        $resultado_adpai->execute();
        $row_adpai = $resultado_adpai->fetch(PDO::FETCH_ASSOC);
        $aditadopai = $row_adpai['aditado_pai'];
        $adtpai =  number_format($aditadopai, 2, ',', '.'); // <--- Total Aditado PAI

    // Total Aditado Passagem Rodoviária
        $result_adrodo ="SELECT SUM(valor_aditivo) as aditado_rodo FROM tbl_aditivo 
        INNER JOIN tbl_item
        ON tbl_aditivo.id_item = tbl_item.id_item WHERE descricao LIKE 'Passagem Rodoviária Nacional' AND tbl_aditivo.status_aditivo LIKE 'ativo'";
        $resultado_adrodo = $conn->prepare($result_adrodo);
        $resultado_adrodo->execute();
        $row_adrodo = $resultado_adrodo->fetch(PDO::FETCH_ASSOC);
        $aditadorodo = $row_adrodo['aditado_rodo'];
        $adtrodo =  number_format($aditadorodo, 2, ',', '.'); // <--- Total Aditado RODO

    // Total Aditado Seguro Viagem Internacional
        $result_adseg="SELECT SUM(valor_aditivo) as aditado_seg FROM tbl_aditivo 
        INNER JOIN tbl_item
        ON tbl_aditivo.id_item = tbl_item.id_item WHERE descricao LIKE 'Seguro Viagem Internacional' AND tbl_aditivo.status_aditivo LIKE 'ativo'";
        $resultado_adseg = $conn->prepare($result_adseg);
        $resultado_adseg->execute();
        $row_adseg = $resultado_adseg->fetch(PDO::FETCH_ASSOC);
        $aditadoseg = $row_adseg['aditado_seg'];
        $adtseg =  number_format($aditadoseg, 2, ',', '.'); // <--- Total Aditado SEG
    // Total Aditado Contrato
        $totaladitado = $aditadopan + $aditadopai + $aditadorodo + $aditadoseg;
        $ttladitado = number_format($totaladitado, 2, ',', '.'); // <--- Total Aditado no Contrato
    // Valor Total Pago
        $result_pago="SELECT SUM(valor_pago) as pago FROM tbl_pagamento 
        INNER JOIN tbl_passagem ON tbl_pagamento.id_passagem = tbl_passagem.id_passagem
        INNER JOIN tbl_contrato ON tbl_passagem.id_contrato = tbl_contrato.id_contrato
        WHERE status_pagamento LIKE 'ativo' AND tbl_contrato.status_contrato LIKE 'ativo'";
    ...
    //VALOR FATURADO GLOBAL
        // Soma Coluna Tarifa Voucher
            $result_tarifa="SELECT SUM(tarifa_voucher) + SUM(taxas_voucher) as total_voucher FROM tbl_passagem
            INNER JOIN tbl_contrato ON tbl_passagem.id_contrato = tbl_contrato.id_contrato
            WHERE status_passagem LIKE 'ativo' AND tbl_contrato.status_contrato LIKE 'vigente'";
        ...
        // Soma Total de Descontos R$
            ...
            WHERE status_passagem LIKE 'ativo'  AND tbl_item.status_item LIKE 'ativo'";
    ...
    //VALOR FATURADO GLOBAL  POR UNIDADE
        // Soma Coluna Tarifa Voucher
            ...
            FROM tbl_passagem 
            INNER JOIN tbl_contrato ON tbl_passagem.id_contrato = tbl_contrato.id_contrato
            WHERE status_passagem LIKE 'ativo' AND unidade = 1 AND tblcontrato.status_contrato LIKE 'ativo'";
        ...
        // Soma Total de Descontos R$
            ...
            LEFT JOIN tbl_passagem
            ON tbl_item.id_item = tbl_passagem.id_item
            WHERE status_passagem LIKE 'ativo' AND unidade = 1 AND tbl_item.status_item LIKE 'ativo'";
    ...
    //Saldo Atualizado do Contrato
        $saldocontrato = $totalcontrato + $totaladitado - $vlrapurado;
    ...
    // SALDOS ITENS
        $saldoPan =  $aereonacional + $aditadopan - $vlrapuradopan;
        $sldPan = number_format($saldoPan, 2, ',', '.'); // <--- Saldo PAN.

        $saldoPai = $aerinternacional + $aditadopai - $vlrapuradopai;
        $sldPai =  number_format($saldoPai, 2, ',', '.'); // <--- Saldo PAI.

        $saldoRodo = $rodonacional + $aditadorodo - $vlrapuradorodo;
        $sldRodo = number_format($saldoRodo, 2, ',', '.'); // <--- Saldo RODO.

        $saldoSeg = $seginternacional + $aditadoseg - $vlrapuradoseg;
        $sldSeg = number_format($saldoSeg, 2, ',', '.'); // <--- Saldo SEG.

  
        

        $result_registros = "SELECT COUNT(id_passagem) AS registro,
        extract(month FROM data_compra) mes,
        extract(year FROM data_compra) ano
    
        FROM tbl_passagem
        WHERE status_passagem ilike 'ativo'
        ORDER BY ano DESC, mes DESC";

        $resultado_registros = $conn->prepare($result_registros);
        $resultado_registros->execute();    

        while ($row_registros = $resultado_registros->fetch(PDO::FETCH_ASSOC)){
            $qdte = $row_registros['registro'];
            $mes = $row_registros['mes'];
            $ano = $row_registros['ano'];
        }
            ...
            <!-- KPIs UTILIZADOS E CONTRATADOS -->
                <div class="row">                    
                    ...
                    <div class="kpi-md-main">
                        ...
                        <span style="font-size: small; color: #123141">Total Aditado:</span> R$ <?=$ttladitado;?><br>
                    ...
                    <div class="kpi-md">
                        ...
                        <span style="font-size: small; color: #123141">Aditado:</span> R$ <?=$adtpan;?><br>
                    ...
                    <div class="kpi-md">
                        ...
                        <span style="font-size: small; color: #123141">Aditado:</span> R$ <?=$adtpai;?><br>
                    ...
                    <div class="kpi-md">
                        ...
                        <span style="font-size: small; color: #123141">Aditado:</span> R$ <?=$adtrodo;?><br>
                    ...
                    <div class="kpi-md">
                        ...
                        <span style="font-size: small; color: #123141">Aditado:</span> R$ <?=$adtseg;?><br>
                            ...
                            var myChart = new Chart(ctx, {
                                ...
                                data: {
                                    ...
                                    datasets: [{
                                        ...
                                        maxBarThickness: 3,
                                        minBarLength: 2,
                                        data: [<?= round(($vlrapurado * 100 / ($totalcontrato + $totaladitado)), 2);?>,<?= round(($saldocontrato * 100 / ($totalcontrato + $totaladitado)), 2);?>],
                                        backgroundColor: [
